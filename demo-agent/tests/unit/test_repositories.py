...
import pytest
from demo_agent.examples.models import User
from demo_agent.examples.repositories import UserRepository
...
def test_save_new_user(user_repository: UserRepository) -> None:
...
def test_save_existing_user(user_repository: UserRepository) -> None:
...
def test_find_by_id(user_repository: UserRepository) -> None:
...
def test_find_by_id_not_found(user_repository: UserRepository) -> None:
    """Tests that find_by_id returns None for a non-existent user."""
    assert user_repository.find_by_id(999) is None


def test_find_by_email(user_repository: UserRepository) -> None:
...
def test_find_by_email_not_found(user_repository: UserRepository) -> None:
